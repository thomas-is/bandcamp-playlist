...
class Process {
    ...
    *
    *   $command MUST be a one single shell command
    ...
    */
    public function __construct($mixed){
        if(is_int($mixed)) $this->id = $mixed;
        if(is_string($mixed)) {
            $id = 0;
            while( file_exists($this->proc($id)) ) $id++;
            if(!mkdir($this->proc($id))) trigger_error("Can't mkdir {$this->proc($id)}",E_USER_ERROR);
            $this->id=$id;
            $this->proc_set("command",$mixed);
        }
    }
    ...
    */
    public function get_title()         { return $this->proc_get("title");          }
    ...
    */
    public function set_title($title)   { return $this->proc_set("title",$title);   }
...
//        if($this->state() == "QUEUED")  return;
        ...
        $path = $this->proc().self::DS;

        if( ! file_exists($this->proc()) ) return;
    ...
    */
    public function start() {
        ...
        // did process exit ?
        if( $this->proc_get("exitcode") !== false) return false;

        // is process already running ?
        if( $this->proc_get("pid") ) return false;

        // does process have a command ?
        $command = $this->proc_get("command");
